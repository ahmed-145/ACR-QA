...
class MetricsCollector:
    ...
    def __init__(self):
        ...
        self._gauges = {}
        self._histograms = {}
        self._histogram_buckets = [0.01, 0.025, 0.05, 0.1, 0.25, 0.5, 1.0, 2.5, 5.0, 10.0]
    ...
    def observe_histogram(self, name: str, value: float, labels: dict = None):
        ...
        with self._lock:
            if key not in self._histograms:
                self._histograms[key] = {"sum": 0, "count": 0, "buckets": {b: 0 for b in self._histogram_buckets}}
...
def record_analysis_metrics(total_findings: int, duration_seconds: float, cache_hits: int = 0, cache_misses: int = 0):

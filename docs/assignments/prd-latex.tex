...
\subsection{Core Innovation (v2.0 Differentiators)}
\begin{enumerate}
    \item \textbf{Canonical Findings Schema:} Universal JSON format normalizes outputs from disparate tools (Ruff, Semgrep, ESLint) across languages
    \item \textbf{RAG-Enhanced Explanations:} Evidence-grounded prompts reduce hallucinations 42-68\% vs. direct LLM calls
    \item \textbf{Provenance-First Architecture:} Stores raw tool outputs, LLM prompts/responses, and user feedback for reproducible evaluation
    \item \textbf{Adapter SDK:} Pluggable language support (Python first, JavaScript/Java next) via standardized interface
\end{enumerate}
...
\textbf{Description:} Universal JSON format normalizing all tool outputs
...
\textbf{Rationale:} Enables language-agnostic dashboard, cross-language comparisons, consistent API
...
\textbf{Description:} Provide developers with access to findings and metrics through appropriate interfaces (not a heavy UI, just access). Includes OWASP Top 10 and SANS Top 25 compliance scoring.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{REST API endpoint: GET /findings}
    \begin{itemize}
        \item Returns canonical findings as JSON
        \item Enables external tools to consume results
        \item No frontend UI required for MVP
    \end{itemize}
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{JavaScript/TypeScript Adapter}
    \begin{itemize}
        \item Gate: Only start if Phase 1 Python precision $\geq$ 80\%
        \item Uses ESLint, similar pattern
    \end{itemize}
\end{itemize}
...
\textit{Phase 2:}
\begin{itemize}[leftmargin=*]
    \item \textbf{Configuration File (.acr-ignore)}
    \begin{itemize}
        \item Repository owners can ignore specific rules or files
        \item Format: Same as .gitignore
        \item Example:
        \begin{verbatim}
tests/ STYLE-*
...
\begin{itemize}[leftmargin=*]
    ...
    \item[$\checkmark$] .acr-ignore file is parsed and honored (Phase 2)
\end{itemize}
...
\textbf{Objective:} Maintain 85\%+ code coverage with focused, fast tests.
...
\textbf{Configuration:}
\begin{itemize}[leftmargin=*]
    \item pytest.ini: min 85\% coverage, fail if lower
    \item GitHub Actions: run tests on every commit
    \item Run time: $<$30 seconds (fast feedback)
\end{itemize}

\textbf{Acceptance:}
\begin{itemize}[leftmargin=*]
    \item[$\checkmark$] 85\%+ code coverage
    \item[$\checkmark$] All tests pass on CI/CD
    \item[$\checkmark$] Test runtime $<$30 seconds
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Setup Time:} 5 minutes from git clone to first analysis (via: \texttt{make setup \&\& make up})
    ...
    \item \textbf{Documentation:} README, architecture docs, API reference, video tutorial
...
\textbf{Future Adapters} (JS, Java, Go) - TBD Phase 2 \\
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Test Coverage:} 45 unit tests across 6 test suites with pytest-cov achieving 70\% threshold
\end{itemize}
...
\textbf{E. Unit Tests}
\begin{itemize}[leftmargin=*]
    \item[$\checkmark$] Code coverage $\geq$85\% (pytest report)
    \item[$\checkmark$] All tests pass on CI/CD
    \item[$\checkmark$] Runtime $<$30 seconds
